...
!developmentChains.includes(network.name)
    ...
    : describe("Dynamic SVG NFT Unit Tests", function () {
          ...
          beforeEach(async () => {
              ...
              dynamicSVGNFTDeployment = await deployments.get("DynamicSVGNFT");
              mockV3AggregatorDeployment = await deployments.get("MockV3Aggregator");
              dynamicSVGNFT = await ethers.getContractAt(dynamicSVGNFTDeployment.abi, dynamicSVGNFTDeployment.address);
          ...
          describe("constructor", () => {
              it("sets starting values correctly", async function () {
                  ...
                  const happySVG = await dynamicSVGNFT.gethappySVG();
                  const priceFeed = await dynamicSVGNFT.getPriceFeed();
                  assert.equal(frownSVG, frownSVGImageURI);
                  assert.equal(happySVG, happySVGimageURI);
          ...
          describe("mint", () => {
              it("emits an event and creates the NFT", async function () {
                  ...
                  await expect(dynamicSVGNFT.mint(threshold))
                      .to.emit(dynamicSVGNFT, "CreatedNFT")
                      .withArgs(0, threshold);
                  const tokenCounter = await dynamicSVGNFT.getTokenCounter();
                  assert.equal(tokenCounter.toString(), "1");
                  const tokenURI = await dynamicSVGNFT.tokenURI(0);
                  assert.equal(tokenURI, happyTokenURI);
              });
              it("shifts the token URI to frowner when the price doesn't surpass the threshold", async function () {
                  const threshold = ethers.parseEther("100000000"); 
                  const transactionResponse = await dynamicSVGNFT.mint(threshold);
                  await transactionResponse.wait(1);
                  const tokenURI = await dynamicSVGNFT.tokenURI(0);
                  assert.equal(tokenURI, frownTokenURI);
              });
          });
      });
